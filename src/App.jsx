...
const CyberAlert = Swal.mixin({
  ...
  color: '#00f2ff',
  didOpen: (toast) => {
    toast.style.border = '1px solid rgba(0, 242, 255, 0.1)';
    toast.style.borderLeft = '4px solid var(--accent)';
    toast.style.borderRight = '1px solid var(--neon-pink)';
    toast.style.fontFamily = 'var(--font-display)';
    toast.style.boxShadow = '0 0 20px rgba(0, 242, 255, 0.2), inset 0 0 15px rgba(157, 0, 255, 0.1)';
    toast.style.backdropFilter = 'blur(12px) saturate(150%)';
    toast.style.borderRadius = '2px';
    toast.style.textTransform = 'uppercase';
    toast.style.letterSpacing = '1px';
    toast.style.backgroundImage = 'repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(0, 242, 255, 0.03) 2px, rgba(0, 242, 255, 0.03) 4px)';

    const progressBar = toast.querySelector('.swal2-timer-progress-bar');
    if (progressBar) {
      progressBar.style.background = 'linear-gradient(90deg, var(--accent), var(--neon-purple), var(--neon-pink))';
      progressBar.style.boxShadow = '0 0 10px var(--accent)';
    }
  }
});
...
function GateLogs({ items, refreshData, showToast }) {
  ...
  const filteredItems = useMemo(() => {
    return items.filter(item => {
      ...
      if (item.createdAt) {
        ...
        // Handle toLocaleString format which can be tricky to parse
        const cleanDate = item.filedDate ? item.filedDate.split(',')[0] : '';
        date = new Date(cleanDate);
      }

      if (isNaN(date.getTime())) {
        // If still invalid, try to extract a date from the filedDate string manually
        const extracted = extractDate(item.filedDate);
        date = extracted !== 'N/A' ? new Date(extracted) : new Date(0);
      }
  ...
  const toggleStatus = async (id, currentStatus) => {
    ...
    if (response.ok) {
      ...
      if (newStatus === 'In Process') {
        showToast('STATUS UPDATED TO IN PROCESS', 'warning', '#ff8c00');
      } else {
        showToast('STATUS UPDATED TO COMPLETED', 'success');
      }
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    if (response.ok) {
      refreshData();
      showToast('RECORD MOVED TO RECOVERY', 'warning', '#fef08a');
    }
...
function GateGraph({ items }) {
  const stats = useMemo(() => {
    ...
    items.forEach(item => {
      // Robust date parsing (same as filteredItems)
      let date;
      if (item.createdAt) {
        date = new Date(item.createdAt);
      } else {
      ...
      if (isNaN(date.getTime())) {
        const dateRegex = /\b\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}\b/;
        const match = item.filedDate ? item.filedDate.match(dateRegex) : null;
        date = match ? new Date(match[0]) : new Date(0);
      }
...
function App() {
  ...
  const showToast = (message, type = 'success', customColor = null) => {
    const defaultColors = { success: 'var(--success)', warning: 'var(--warning)', error: '#ef4444' };
    const color = customColor || defaultColors[type] || defaultColors['success'];

    // Split the message to style the last word(s)
    let words = message.split(' ');

    // Handle multi-word endings like "IN PROCESS"
    let lastWord = words.pop();
    if (lastWord === 'PROCESS' && words[words.length - 1] === 'IN') {
      lastWord = words.pop() + ' ' + lastWord;
    }

    const restOfMessage = words.join(' ');
    const styledMessage = `${restOfMessage} <span style="color: ${color}; text-shadow: 0 0 10px ${color};">${lastWord}</span>`;
